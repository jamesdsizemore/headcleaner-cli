...
# Build:   docker build -t headcleaner:0.4.0 .
# Run:     docker run --rm -v /path/to/inbox:/inbox -v /path/to/out:/out headcleaner:0.4.0 convert /inbox --output /out
#
...
# Install deps into a venv we copy to runtime
RUN uv sync --frozen --no-install-project --all-extras --no-dev
...
RUN uv pip install --python /build/.venv/bin/python /build/dist/*.whl
...
LABEL org.opencontainers.image.source="https://github.com/local/headcleaner"
LABEL org.opencontainers.image.licenses="Apache-2.0"

WORKDIR /app

# Tesseract for OCR (optional; only used with --ocr)
RUN apt-get update && apt-get install -y --no-install-recommends \
    tesseract-ocr \
    && rm -rf /var/lib/apt/lists/*
