"""OKF bundle viewer (v0.10.0).

Adopted from `scaccogatto/okf-skills` (MIT, 312 stars). Vendored as
``src/headcleaner/viewer.py`` with the original logic preserved verbatim and
a Python API added on top. The HTML/CSS/JS template is byte-identical to the
upstream ``okf_visualize.py`` so the rendered output matches what users get
from the upstream skill.

.. note::
    Upstream copyright (c) scaccogatto. Licensed under MIT.
    See ``docs/ATTRIBUTION.md`` for the full notice and the diff
    between this file and the upstream source.

What this module provides, beyond the upstream CLI:

- A ``render(...)`` function callable from Python (the upstream CLI was the
  only entry point).
- A ``render_to_string(...)`` helper for tests + library users who don't
  want to write to disk.
- The whole module exposes ``build()`` and ``split_frontmatter()`` so other
...
# PyYAML is an optional dependency for headcleaner-core (only required by the
# viewer + linter). Listed in pyproject.toml as `pyyaml>=6`.
try:
    import yaml  # noqa: F401  (already imported above; this is for the badge)
except ImportError as e:  # pragma: no cover
    raise ImportError(
        "headcleaner.viewer requires PyYAML. Install it with "
        "`uv pip install pyyaml` or `uv add pyyaml`."
    ) from e
...
<div id="bar">
 ...
 <select id="layout">
  ...
  <option value="breadthfirst">breadth-first</option><option value="circle">circle</option>
...
def render(
    ...
    this function is the canonical Python entry point.
    """
    nodes, edges = build(bundle)
    if max_nodes is not None and len(nodes) > max_nodes:
        raise ValueError(f"{len(nodes)} concepts exceeds max_nodes {max_nodes}")
    if layout is None:
...
def render_to_string(nodes, edges, *, title, link=None, layout="cose", og_image=None):
    ...
    return marker.sub(lambda m: subs[m.group(0)], HTML)
